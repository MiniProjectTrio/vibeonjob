...
export default function Dashboard() {
  ...
  const handleAnalysisComplete = (result) => {
    ...
    fetchStats(); // Update analysis count
  }
    ...
    <div className="selection:bg-blue-100 selection:text-blue-900 bg-[#f8fafc] font-body min-h-screen">
      ...
      <aside className="fixed left-0 top-0 h-full w-64 hidden lg:flex flex-col border-r border-slate-100 bg-white mt-20 z-40">
        <div className="flex flex-col gap-1 p-6">
          ...
          <Link to="/about" className="flex items-center gap-3 text-slate-500 hover:bg-slate-50 rounded-xl px-4 py-3 font-medium text-[14px]">
            <span className="material-symbols-outlined text-[20px]">info</span> About Us
          </Link>
        </div>
      ...
      {/* Main Content */}
      <main className="pt-24 pb-20 px-4 lg:pl-72 lg:pr-8 mx-auto w-full max-w-screen-2xl">
        <div className="">
          {/* Welcome Header */}
              ...
              <div className="bento-card rounded-xl p-10 flex flex-col justify-between relative overflow-hidden group">
                <div>
                  ...
                  <div className="mt-4 flex items-baseline">
                    <span className="editorial-headline text-[80px] font-extrabold gradient-text leading-none">{stats?.total_analyses ?? 0}</span>
                  </div>
                </div>
